...
<div class="bg-white p-6 rounded-xl shadow">
    ...
    <div class="mb-6 p-4 border rounded bg-gray-50">
        ...
        <p class="font-bold mb-2">
            📄 Template & Panduan Pengisian
        </p>

        <p class="text-sm text-gray-600 mb-3">
            Gunakan template berikut untuk menghindari kesalahan format data.
            Jangan mengubah urutan kolom Excel.
        </p>
    ...
    {{-- SELECT JENIS SURAT --}}
    {{-- ===================== --}}
    <form method="GET" action="{{ route('user.pengajuan') }}" class="mb-6">
        ...
        <select name="jenis_surat_id"
            ...
            @foreach($jenisSurat as $js)
                <option value="{{ $js->id }}"
                    {{ request()->input('jenis_surat_id') == $js->id ? 'selected' : '' }}>
                    {{ $js->nama_jenis }}
                </option>
            @endforeach
    ...
    @if(!$selectedSurat)
        <div class="text-gray-500 text-center py-10">
            Silakan pilih jenis surat untuk memulai pengajuan.
        </div>
    @endif
...
window.autoPreviewExcel = function(id) {
    ...
    if (!input || !preview) {
        console.error("INPUT / PREVIEW TIDAK DITEMUKAN");
        return;
    }
    ...
    reader.onload = function (e) {
        ...
        rows.forEach(row => {
            ...
            headers.forEach((_, i) => {
                html += `<td class="border p-2">${row[i] ?? ''}</td>`;
            });
